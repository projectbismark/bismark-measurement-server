...
compile_repo()
{
    ...
    mkdir -p "$webdir"/i686
    find "$rpmbuild_rpms"/i686/ -mindepth 1 -maxdepth 1 ! -name '*-debuginfo-*' \
            -exec cp '{}' "$webdir"/i686/ \;
    cp -a "$rpmbuild_rpms"/noarch/*.rpm "$webdir"/i686/
    createrepo -o "$webdir"/i686 -v -d "$webdir"/i686
    $sign_rpms && gpg -a --detach-sign "$webdir"/i686/repodata/repomd.xml

    # copy debuginfo RPMs & make repodata
    mkdir -p "$webdir"/i686/debug
    find "$rpmbuild_rpms"/i686/ -mindepth 1 -maxdepth 1 -name '*-debuginfo-*' \
            -exec cp '{}' "$webdir"/i686/debug/ \;
    createrepo -o "$webdir"/i686/debug -v -d "$webdir"/i686/debug
    $sign_rpms && gpg -a --detach-sign "$webdir"/i686/debug/repodata/repomd.xml
